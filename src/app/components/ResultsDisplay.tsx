...
export default function ResultsDisplay({ extractionResult, isProcessing }: ResultsDisplayProps) {
  ...
  const handleExportCSV = () => {
    ...
    const header = ['Attribute', 'Value', 'Confidence'];
    const csvLines = [
      header.join(','),
      ...rows.map((r) =>
        [r.Attribute, `"${(r.Value ?? '').replace(/"/g, '""')}"`, r.Confidence].join(',')
      ),
      '',
  ...
  const handleExportExcel = () => {
    const rows = buildRows();
    const summaryRows = [
      { Attribute: 'Overall Score', Value: `${percentageScore}%`, Confidence: statusText },
    ];
    ...
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6">
        ...
        <div className={`${statusBgColor} rounded-lg p-3 sm:p-4 border-2 ${statusBorderColor}`}>
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className={`text-3xl sm:text-5xl font-bold mb-1 ${statusColor}`}>
                {percentageScore}%
              </div>
              <div className="text-xs sm:text-sm text-slate-600">
                {highConfidenceCount}/10 High Confidence Attributes
              </div>
            </div>
      ...
      <div className="mt-5 sm:mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-semibold text-sm"
        >
